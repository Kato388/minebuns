...
.mb-wm-ver{color:rgba(0,255,80,0.3);font-size:9px;margin-left:6px;}
...
var k=class extends a{
  constructor(){super("Watermark","Visual",{Text:"Minebuns"});this._clockInterval=null;}
...
var S=class{
  ...
  // keybind ラベル表示用マップ
  static keyLabels={"KeyG":"G","KeyK":"K","KeyF":"F","KeyS":"S","KeyN":"N","KeyC":"C","KeyI":"I","ShiftRight":"SHIFT"};
  addButton(mod){
...
// NoClip: Nキー
var N=class extends a{constructor(){super("NoClip","Movement",null,"KeyN");}get pps(){return o.gameWorld.systemsManager.activeSystems.find(s=>s?.playerPhysicsSystem).playerPhysicsSystem;}onRender(){if(!o?.gameWorld?.player)return;this._og=this._og||this.pps.resolveBlockCollision;if(this.pps.resolveBlockCollision==this._og)this.pps.resolveBlockCollision=()=>{};}onDisable(){if(this._og)this.pps.resolveBlockCollision=this._og;}};
...
// Spider: Sキー
var q=class extends a{constructor(){super("Spider","Movement",{Speed:5},"KeyS");}get pps(){return o.gameWorld.systemsManager.activeSystems.find(s=>s?.playerPhysicsSystem).playerPhysicsSystem;}get upAgainstWall(){return this.pps.playerVelVec3.x==0||this.pps.playerVelVec3.z==0;}onRender(){if(o?.gameWorld?.player&&o?.gameWorld?.player?.inputs.jump&&this.upAgainstWall)o.gameWorld.player.velocity.velVec3.y=this.options.Speed;}};
...
var Kb=class extends a{
  ...
  _buildUI(){
    ...
    const binds=[
      ...
      ["N","NoClip"],["C","Chams"],["S","Spider"],["SHIFT","ClickGUI"]
    ];
...
var Nv=class extends a{
  constructor(){super("NightVision","Visual",{"Brightness":5});}
  _getRenderer(){return o?.gameWorld?.systemsManager?.activeSystems?.find(s=>s?.renderer)?.renderer||o?.gameWorld?.renderer||null;}
  onEnable(){
    if(!o?.gameWorld)return;
    try{
      let cm=o.gameWorld.chunkManager;
      if(cm){
        this._origAmbient=this._origAmbient??cm.ambientLight;
        this._origSkyBrightness=this._origSkyBrightness??cm.skyBrightness;
        cm.ambientLight=parseFloat(this.options.Brightness);
        if(cm.skyBrightness!==undefined)cm.skyBrightness=parseFloat(this.options.Brightness);
      }
    }catch(e){}
    try{
      let scene=o.gameWorld.scene;
      if(scene?.fog){this._origFogEnabled=this._origFogEnabled??true;scene.fog=null;}
    }catch(e){}
  }
  onDisable(){
    try{
      let cm=o.gameWorld.chunkManager;
      if(cm){
        if(this._origAmbient!==undefined)cm.ambientLight=this._origAmbient;
        if(this._origSkyBrightness!==undefined&&cm.skyBrightness!==undefined)cm.skyBrightness=this._origSkyBrightness;
      }
    }catch(e){}
    try{
      let scene=o.gameWorld.scene;
      if(scene&&this._origFogEnabled)scene.fog=this._origFogEnabled;
    }catch(e){}
  }
  onSettingUpdate(){if(this.isEnabled)this.onEnable();}
};
...
var h={
  ...
  init(){
    this.addModules(v,k,M,C2,W,w,B,I,P,K,A,Fl,D,R2,T,Gm,O,N,L,U,H,V,F2,j,G2,Y,q,X,Z,ee,te,oe,Q,Dy,Ct,Kb,Nv);
...
var se=class{
  ...
  init(){
    ...
    document.addEventListener("keydown",e=>c.emit("keydown",e.code));
    h.init();p.init();
